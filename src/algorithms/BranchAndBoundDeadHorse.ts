...
export function branchAndBoundDeadHorse(graph: Graph, startId: string, goalId: string): PathResult {
  ...
  while (queue.length > 0) {
    ...
    for (const neighbor of graphUtil.getNeighbors(currentNode)) {
      if (!visited.has(neighbor.node)) {
        if (neighbor.weight !== undefined && neighbor.weight > 10) {
          continue; // Apply the Dead Horse principle, skip paths with weight > 10
        }
